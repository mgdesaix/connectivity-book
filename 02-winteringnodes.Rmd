...
```{r, warning = F, message=F}
winter_coords <- terra::vect(mignette::amre_nonbreeding_data, 
                             geom=c("Lon", "Lat"),
                             crs = "EPSG:4326")
winter_coords_nearest <- terra::nearest(winter_coords,
                                        ecoregions,
                                        centroids = FALSE)
# Add Region ID from original file for easier interpretation
winter_coords_nearest$Region <- ecoregions$Region[winter_coords_nearest$to_id]
table(winter_coords_nearest$Region)
```
...
ecoregions_keep <- data.frame(table(winter_coords_nearest$Region)) %>%
  filter(!.[2] < cutoff) %>%
  pull(1) %>%
  as.character()

winter_coords_nearest_subset <- winter_coords_nearest[winter_coords_nearest$Region %in% ecoregions_keep]
winter_regions_subset <- ecoregions[winter_coords_nearest_subset]
```
...
```{r, warning = F, message = F}
ggplot() +
  geom_spatvector(data = winter_regions_subset, aes(fill = Region)) +
  geom_spatvector(data = winter_coords_nearest_subset) +
  theme_bw()
```
